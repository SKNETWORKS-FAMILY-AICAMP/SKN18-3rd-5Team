...
def run(state: QAState) -> QAState:
    ...
    print(f"[QueryRewrite] start (question={q[:120] if q else ''})")
    ...
    if temporal_info and temporal_info.filters:
        print(f"[QueryRewrite] temporal filters={temporal_info.filters}")
    print(f"[QueryRewrite] complete (rewritten={rewritten[:120] if rewritten else ''})")
    return state
