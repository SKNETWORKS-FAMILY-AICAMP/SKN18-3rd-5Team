...
"""
임베딩 생성 스크립트
PostgreSQL의 document_chunks 테이블을 읽어 임베딩을 생성하고 embeddings_* 테이블에 저장

사용법:
    # ETL loader 디렉토리에서 실행
    cd service/etl/loader
    python generate_embeddings.py --model e5 --batch-size 100
    ...
    def _get_chunks_to_process(
        ...
        if skip_existing:
            ...
            sql = f"""
                ...
                LEFT JOIN vector_db.{embedding_table} e ON c.id = e.chunk_id
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="PostgreSQL document_chunks → 임베딩 생성 → embeddings_* 테이블 저장",
